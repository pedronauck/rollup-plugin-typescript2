...
export default function typescript(options?: Partial<IOptions>): any {
  ...
  return {
    ...
    options(config: IRollupOptions): any {
      ...
      if (
        !satisfies(tsModule.version, '^3.2.0', {
        ...
        throw new Error(
          `Installed typescript version '${
            tsModule.version
          }' is outside of supported range '^3.2.0'`
        )

      context.info(`rollup-plugin-typescript2 version: 0.18.3`)
      context.debug(
        () =>
          `plugin options:\n${JSON.stringify(
            pluginOptions,
            (key, value) =>
              key === 'typescript'
                ? `version ${(value as typeof tsModule).version}`
                : value,
            4
          )}`
      )
      ...
      service = tsModule.createLanguageService(
        servicesHost,
    ...
    load(id: string): string | undefined {
      if (id === '\0' + TSLIB) return tslibSource
    ...
    generateBundle(bundleOpts: any, bundle: any, isWrite: any): void {
      const { dir, file } = bundleOpts

      if (isWrite) {
        if (parsedConfig.options.declaration) {
          _.each(parsedConfig.fileNames, name => {
            const key = normalize(name)
            if (_.has(declarations, key) || !filter(key)) return
          ...
          const bundleFile = file ? file : dir
          ...
          const writeDeclaration = (
            ...
            context.debug(
              () =>
                ...
                )} for '${key}' to '${writeToPath}'`
            )

            // Write the declaration file to disk.
            tsModule.sys.writeFile(
              writeToPath,
              entry.text,
              entry.writeByteOrderMark
            )
          }
