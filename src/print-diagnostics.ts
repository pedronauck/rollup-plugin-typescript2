...
export function printDiagnostics(
  ...
  _.each(diagnostics, diagnostic => {
    ...
    if (pretty) print.call(context, [`${diagnostic.formatted}`] as any)
    else {
      if (diagnostic.fileLine !== undefined)
        print.call(context, [
          `${diagnostic.fileLine}: ${type}${category} TS${
            diagnostic.code
          } ${color(diagnostic.flatMessage)}`,
        ] as any)
      else
        print.call(context, [
          `${type}${category} TS${diagnostic.code} ${color(
