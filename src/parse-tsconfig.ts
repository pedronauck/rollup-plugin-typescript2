...
export function parseTsConfig(
  ...
  if (fileName) {
    ...
    if (result.error !== undefined) {
      printDiagnostics(
        context,
        convertDiagnostic('config', [result.error]),
        pretty
      )
  ...
  const compilerOptionsOverride = getOptionsOverrides(
    pluginOptions,
    preParsedTsConfig
  )
  ...
  printDiagnostics(
    context,
    convertDiagnostic('config', parsedTsConfig.errors),
    pretty
  )

  context.debug(
    `built-in options overrides: ${JSON.stringify(
      compilerOptionsOverride,
      undefined,
      4
    )}`
  ...
  context.debug(
    `parsed tsconfig: ${JSON.stringify(parsedTsConfig, undefined, 4)}`
  )
