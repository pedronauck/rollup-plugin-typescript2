...
export function convertEmitOutput(output: tsTypes.EmitOutput): ICode {
  ...
  output.outputFiles.forEach(e => {
    if (_.endsWith(e.name, '.d.ts')) out.dts = e
    else if (_.endsWith(e.name, '.d.ts.map')) out.dtsmap = e
    else if (_.endsWith(e.name, '.map')) out.map = e.text
...
export function convertDiagnostic(
  type: string,
  data: tsTypes.Diagnostic[]
): IDiagnostics[] {
  return _.map(data, diagnostic => {
    const entry: IDiagnostics = {
      flatMessage: tsModule.flattenDiagnosticMessageText(
        diagnostic.messageText,
        '\n'
      ),
      formatted: tsModule.formatDiagnosticsWithColorAndContext(
        data,
        formatHost
      ),
    ...
    if (diagnostic.file && diagnostic.start !== undefined) {
      ...
      entry.fileLine = `${diagnostic.file.fileName}(${line + 1},${character +
        1})`
    }
...
export class TsCache {
  ...
  constructor(
    ...
    this.dependencyTree.setDefaultNodeLabel((_node: string) => ({
      dirty: false,
    }))

    const automaticTypes = _.map(
      tsModule.getAutomaticTypeDirectiveNames(options, tsModule.sys),
      entry =>
        tsModule.resolveTypeReferenceDirective(
          entry,
          undefined,
          options,
          tsModule.sys
    ...
    this.ambientTypes = _.filter(rootFilenames, file =>
      _.endsWith(file, '.d.ts')
    )
  ...
  public getSyntacticDiagnostics(
    ...
    return this.getDiagnostics(
      'syntax',
      this.syntacticDiagnosticsCache,
      id,
      snapshot,
      check
    )
  ...
  private checkAmbientTypes(): void {
    ...
    if (this.ambientTypesDirty)
      this.context.info(
        yellow('ambient types changed, redoing all semantic diagnostics')
      )
